	...
	exports.TeambuilderRoom = exports.Room.extend({
		...
		createTeam: function (orig, isBox) {
			...
			if (orig) {
				...
				var format = this.curFolder || 'gen9testingstandard';
				var folder = '';
				if (format && format.charAt(format.length - 1) === '/') {
					folder = format.slice(0, -1);
					format = 'gen9testingstandard';
				}
				newTeam = {
		...
		clipboardInnerHTML: function () {
			...
			for (var i = 0; i < this.clipboardCount(); i++) {
				...
				buf += '<span class="species">' + (species.name === species.baseSpecies || !species.name.startsWith(species.baseSpecies + '-') ? BattleLog.escapeHTML(species.name) : (BattleLog.escapeHTML(species.baseSpecies) + '-<small>' + BattleLog.escapeHTML(species.name.substr(species.baseSpecies.length + 1)) + '</small>')) + '</span></div>';
		...
		statChange: function (e) {
			...
			var usesStatPoints = (this.curTeam.format.includes('champions') || this.curTeam.format.includes('testingstandard'));
		...
		updateDetailsForm: function () {
			var buf = '';
			var set = this.curSet;
			var isChampions = (this.curTeam.format.includes('champions') || this.curTeam.format.includes('testingstandard'));
		...
		chooseMove: function (moveName, resetSpeed) {
			...
			if (this.curTeam.format.includes('1v1') || this.curTeam.format.includes('categoryswap') ||
				...
				(this.curTeam.format.includes('champions') || this.curTeam.format.includes('testingstandard'))) return;
		...
		setPokemon: function (val, selectNext) {
			...
			if (this.curTeam && this.curTeam.format) {
				...
				if (!baseFormat) baseFormat = 'ou';
				if (this.curTeam && this.curTeam.format) {
					if (baseFormat.substr(0, 9) === 'champions' || baseFormat.substr(0, 15) === 'testingstandard' || baseFormat.substr(0, 10) === 'battlespot' ||
