...
class BattlePanel extends PSRoomPanel<BattleRoom> {
	...
	toggleBoostedMove = (e: Event) => {
		...
		case 'telepathy':
			...
			break;
		case 'quicktunnel':
			choices.current.quickTunnel = checkbox.checked;
			break;
		}
	...
	renderMoveButton(props: {
		...
		>
			{props.name}<br />
			<small class="type">{Dex.types.get(props.type).name} <span class="effectiveness-icon">{props.tags}</span></small> {}
	...
	renderMoveMenu(choices: BattleChoiceBuilder) {
		...
		const canQuickTunnel = moveRequest.canQuickTunnel;
